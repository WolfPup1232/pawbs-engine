...
export default function initializeEditorUIEventHandlers(world, player)
{
	...
	//#region [Editor Main Menu]
		...
		//#region [Sub-Editors (Modes)]
			
			/**
			 * Editor mode type selected checkbox change event.
			 */
			$('input[type="checkbox"][name="editor-mode-types"]').change(function()
			{
				if (Editor.enabled)
				{
					
					// Get editor mode type selected checkbox value
					let selected_value = $(this).val();
					
					// Change editor mode based on selected checkbox value
					if (selected_value === 'prefabs')
					{
		...
		//#region [Tools]
			...
			$('input[type="checkbox"][name="editor-tool-types"]').change(function()
			{
				if (Editor.enabled)
				{
					...
					// Change editor tool based on selected checkbox value
	...
	//#region [Editor Spawn Tool]
		...
		// TODO: Spawn tool events go here.
